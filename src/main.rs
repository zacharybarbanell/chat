...
fn main() {
	...
	for message in rx {
		match message {
			...
			Message::Close(n) => {
				transmitters.remove(&n).unwrap().send(Message::Close(0)).unwrap();
			}
		}
	}
}

fn handle_connection(mut stream : TcpStream, id : i32, tx : mpsc::Sender<Message>, rx : mpsc::Receiver<Message>) {
...
fn chat(mut stream : TcpStream, id : i32, tx : mpsc::Sender<Message>, rx : mpsc::Receiver<Message>) {
	...
	loop {
		...
		partial.extend(text);
		if fin {
			tx.send(Message::Chat(String::from_utf8(partial).unwrap()));
			partial = Vec::new();
		}
	}
}
